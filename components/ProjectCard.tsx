      ...
      {project.topics.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {project.topics.slice(0, 3).map((t) => (
            <span
              key={t}
              className="border border-border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-text-muted transition-colors group-hover:border-primary/40"
            >
              {t}
            </span>
          ))}
        ...
        <span className="flex items-center gap-2 text-text-muted">
          {project.lang && (
            <span
              ...
              style={{ backgroundColor: langColor[project.lang] ?? "var(--text-muted)" }}
            />
          )}
          {project.lang}
        </span>
